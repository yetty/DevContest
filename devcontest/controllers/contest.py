...
from devcontest.model import Source, Contest, Task, \
        contests_table, sources_table, tasks_table
...
class ContestController(BaseController):
    ...
    def rank(self, id):
        ...
        if not self.contest.results and self.contest.is_running \
            ...
            return redirect(url(action="tasks"))
    ...
    def timeSolved(self, id):
        s = Session.query(Source). \
                filter_by(task_id=id, user_id=self.user.id, status=True). \
                first()
    ...
    def toIndex(self, id=None):
        return redirect(url(
            controller="admin", action="contest", id=id, param=None
        ))
    ...
    def admin(self, id=None, param=None):
        ...
        if param == "save":
            ...
            return redirect(url(param=None))
        ...
        if param == "create_task":
            ...
            Session.commit()
            return redirect(url(
                        controller="admin", action="task", id=t.id, param=None
                    ))
    ...
    def _remove(self, id):
        Session.execute(contests_table.delete().
                        where(contests_table.c.id == int(id)))
        Session.execute(tasks_table.delete().
                        where(tasks_table.c.contest_id == int(id)))
        Session.execute(sources_table.delete().
                        where(sources_table.c.contest_id == int(id)))
        Session.commit()
