...
from pylons import request, response, tmpl_context as c, config, url
from pylons.controllers.util import redirect
from pylons.i18n import _

from devcontest.lib.base import BaseController, render
from devcontest.model import Source, Runner, Judge, judges_table, \
                             Task, Contest, tasks_table, sources_table
...
class TaskController(BaseController):
    ...
    def show(self, id, param=None):
        ...
        self.source = Session.query(Source). \
                filter_by(user_id=self.user.id, task_id=id).first()
        if self.source:
            self.source.load()
    ...
    def _upload(self):
        ...
        try:
            ...
            if request.POST['code'] != '' and request.POST['type'] != '*':
                fileName = b16encode(request.POST['code'])[:16] + \
                                "." + request.POST['type']
        ...
        if not self.source:
            self.source = Source(self.task.contest_id,
                                 self.task.id, self.user.id, fileName)
    ...
    def admin(self, id=None, param=None, num=None):
        ...
        if param == "deljudge" and num is not None:
            Session.execute(judges_table.delete().
                            where(judges_table.c.id == int(num)))
            Session.commit()
        ...
        if param == "remove":
            ...
            self._remove()
            return redirect(url(action="contest", id=contest_id, param=None))

        c.task = self.task
        if self.task.source:
            c.runner = Session.query(Runner). \
                        filter_by(lang=self.task.filetype).one()
        c.contest = self.contest
        c.runners = Session.query(Runner).all()
    ...
    def _saveTmpIn(self, data):
        nameIn = os.path.join(config.get('runner_tmp_dir'),
                              hashlib.sha256(str(time.time()) +
    ...
    def _remove(self):
        Session.execute(tasks_table.delete().
                        where(tasks_table.c.id == self.task.id))
        Session.execute(sources_table.delete().
                        where(sources_table.c.task_id == self.task.id))
        Session.commit()

    def _load(self, id):
        self.task = Session.query(Task).filter_by(id=id).first()
        self.contest = Session.query(Contest). \
                        filter_by(id=self.task.contest_id).one()
        self.task.load()
    ...
    def _save(self):
        ...
        for i in range(int(request.POST['count'])):
            ...
            if 'file_in[' + str(i) + ']' in request.POST \
                    and request.POST['file_in[' + str(i) + ']'] != '':
                judge = Judge(
                    self.task.id,
                    points=points,
                    time_limit=request.POST['time_limit[' + str(i) + ']'],
                    memory_limit=request.POST['memory_limit[' + str(i) + ']']
                )
            ...
            elif 'id[' + str(i) + ']' in request.POST:
                judge = Session.query(Judge).filter_by(
                            id=request.POST['id[' + str(i) + ']']
                        ).one()
                judge.points = points
                judge.time_limit = request.POST['time_limit[' + str(i) + ']']
                judge.memory_limit = request.POST['memory_limit['
                                                  + str(i) + ']']
        ...
        if r:
            if nolimit:
                ...
                get = r.exe(file, fileIn, i=i,
                            ...
                            memory_limit=self.task.memory_limit)
            return get
        else:
            return {'errors': _("This script need support of *.%s") % lang,
                    'return': '', 'status': 'false', 'compile': ''}
