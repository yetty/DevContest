...
class PageController(BaseController):
    ...
    def index(self, id=None):
        if not id:
            return redirect(url(controller="home", action="index", id=None))
    ...
    def _filename(self):
        return os.path.join(config.get('page_dir'),
                            self.page + "." + self.extension)
    ...
    def admin(self, id=None, param=None):
        ...
        if param == "remove":
            self._remove()
            return redirect(url(id=None, param=None))

        if id == "_" and param == "create":
            self._create(request.params['url'])
            return redirect(url(id=self.page, param=None))
