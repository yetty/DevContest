...
from devcontest.model import Source, User, Contest, users_table, Task, \
                             Runner, Judge, sources_table
...
class UserController(BaseController):
    ...
    def source(self, id=0):
        ...
        c.source = Session.query(Source). \
                filter_by(id=id, user_id=self.user.id).first()
        if not c.source:
            return redirect(url(action="sources", id=None))
    ...
    def top(self, id=10):
        ...
        c.users = []
        for user in users:
            count = Session.query(Source). \
                    filter_by(user_id=user.id, status=True). \
                    filter(not_(Source.contest_id.in_(runningContests))) \
                    .count()
    ...
    def admin(self, id, param):
        ...
        if id == "remove" and param:
            Session.execute(users_table.delete().
                            where(users_table.c.id == int(param)))
            Session.commit()
        ...
        if id == "source_rerun" and param:
            ...
            contest = Session.query(Contest). \
                    filter_by(id=c.source.contest_id).first()
            ...
            Session.commit()
            return redirect(url(id="source_view"))
        ...
        if id == "sources" and param:
            c.user = Session.query(User).filter_by(id=int(param)).first()
            c.sources = Session.query(Source). \
                    filter_by(user_id=int(param)). \
                    order_by(sources_table.c.datetime.desc()).all()
            c.getTaskName = self._getTaskName
        ...
        c.users = Session.query(User). \
                order_by(users_table.c.lname, users_table.c.fname).all()
    ...
    def _adminSave(self, id, params):
        ...
        user.cls = params['cls']
        if params['password'] != '' \
           and params['password'] == params['cpassword']:
